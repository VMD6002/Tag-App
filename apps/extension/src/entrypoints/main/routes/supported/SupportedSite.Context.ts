...
export const defaultSiteScript = `ContentData.title = document.title;
...
function useSupportedSite() {
  ...
  }, [siteData, siteScript, editingSiteName]);

  const selectSite = useCallback((site: SiteData) => {
    try {
      const { script, ...otherSiteData } = site;
...
export const [
  ...
  ({ siteDataEditorOpen, siteData, setSiteData, siteScript, setSiteScript, editingSiteName }) => ({
    siteDataEditorOpen,
    siteData,
    setSiteData,
    siteScript,
